...
export const isNodeWithSteps = (
  node: BaseModels.BaseDiagramNode
): node is BaseModels.BaseBlock | BaseModels.BaseActions => {
  return Array.isArray(node.data.steps) && node.data.steps.length > 0;
};

export const isStep = <
  S extends BaseModels.AnyBaseStep = BaseModels.AnyBaseStep
>(
  node: BaseModels.BaseDiagramNode
): node is S => {
...
export const getSteps = <
  S extends BaseModels.AnyBaseStep = BaseModels.AnyBaseStep
>(
  diagram: BaseModels.Diagram.Model
) => {
  const steps: S[] = [];
